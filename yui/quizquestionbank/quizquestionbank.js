YUI.add('moodle-mod_branchedquiz-quizquestionbank', function (Y, NAME) {
    ...
    Y.extend(POPUP, Y.Base, {
        ...
        load_content : function(queryString) {
            Y.log('Starting load.', 'debug', 'moodle-mod_branchedquiz-quizquestionbank');
            this.dialogue.bodyNode.append(this.loadingDiv);

            // If to support old IE.
            if (window.history.replaceState) {
                window.history.replaceState(null, '', M.cfg.wwwroot + '/mod/branchedquiz/edit.php' + queryString);
            }

            Y.io(M.cfg.wwwroot + '/mod/branchedquiz/questionbank.ajax.php' + queryString, {
                method: 'GET',
                on: {
            ...
            Y.log('Load request sent.', 'debug', 'moodle-mod_branchedquiz-quizquestionbank');
        },
        ...
        load_done: function(transactionid, response) {
            ...
            Y.log('Load completed.', 'debug', 'moodle-mod_branchedquiz-quizquestionbank');
        ...
        load_failed: function() {
            Y.log('Load failed.', 'debug', 'moodle-mod_branchedquiz-quizquestionbank');
        },
    ...
    M.mod_branchedquiz.quizquestionbank = M.mod_branchedquiz.quizquestionbank || {};
    M.mod_branchedquiz.quizquestionbank.init = function() {
        return new POPUP();
    };
}, '@VERSION@', {
    "requires": [
